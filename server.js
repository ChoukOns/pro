...
const validateObjectId = (req, res, next) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id)) {
    return res.status(400).json({ message: "ID de l'apprenant non valide" });
  }

  req.params.id = mongoose.Types.ObjectId(id); // Convertir l'ID en ObjectId
  next();
};

app.get('/api/apprenants/:id', validateObjectId, async (req, res) => {
  try {
    const { id } = req.params;
...
app.post('/api/acheter-cours', async (req, res) => {
  const { apprenantId, coursId, montant, dateA } = req.body;

  if (!mongoose.Types.ObjectId.isValid(apprenantId) || !mongoose.Types.ObjectId.isValid(coursId)) {
    return res.status(400).json({ message: "ID d'apprenant ou ID de cours non valide" });
  }
  ...
  try {
    ...
    // Logique pour ajouter l'achat de cours...

    res.json({ message: "Cours acheté avec succès" });
  } catch (error) {
    res.status(500).json({ message: "Erreur lors de l'achat du cours", error: error.message });
  }
});
